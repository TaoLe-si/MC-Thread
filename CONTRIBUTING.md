...
- **Gradle 8.14.5**：使用仓库内 Wrapper（`gradlew.bat`），无需手动安装；
- **网络说明（CN 网络）**：
...
1. 从 `master` 拉取最新，创建功能分支：`feature/<主题>`；
2. 实现并补充测试；
3. 本地验证：`gradlew test build` 必须全绿；涉及游戏内行为的改动运行 `gradlew runGameTestServer`；
...
- `InteractionExecutor` 任务只处理可延后、可批处理的工作（IO、序列化、批量通知）。
...
- 新 Mixin 必须能解释清楚失效/回滚路径（如能力缓存的 `invalidateCaps/reviveCaps` 清空）。
