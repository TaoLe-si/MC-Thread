package com.taolesi.threadtearer;

import com.mojang.logging.LogUtils;
import com.taolesi.threadtearer.adapter.SyntheticCraftAdapter;
import com.taolesi.threadtearer.command.MCTCommand;
import com.taolesi.threadtearer.config.MCThreadConfig;
import com.taolesi.threadtearer.gametest.MCTGameTests;
...
public final class MCThread {
    ...
    public static final String MODID = "threadtearer";
    private static final Logger LOGGER = LogUtils.getLogger();
    private final String actualVersion;

    public MCThread(IEventBus modEventBus, ModContainer container) {
        this.actualVersion = container.getModInfo().getVersion().toString();
        new MCTRuntimeImpl();
        com.taolesi.threadtearer.api.MCT.install(MCTRuntimeImpl.get());
    ...
    private void commonSetup(final FMLCommonSetupEvent event) {
        LOGGER.info("[Thread Tearer] 1.21.1-neoforge loaded (mod_version={})", actualVersion);
        LOGGER.info("[Thread Tearer] runtime.enabled={}, computeThreads={}, profiler.enabled={}, experiments.offloadPlayerUseItem={}",
        ...
        LOGGER.info("[Thread Tearer] commands registered: /threadtearer (prof/replay/bench/monitor/adapters/runtime/demo/experiment/selftest)");
        LOGGER.info("[Thread Tearer] mixin config=threadtearer.mixins.json (look for Selecting config threadtearer.mixins.json in debug.log)");
    }
