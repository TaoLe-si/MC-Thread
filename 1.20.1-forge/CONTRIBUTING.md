...
仓库按 **Minecraft 版本 + 加载器** 拆分。当前实现在分支 `1.20.1-forge`、目录 `1.20.1-forge/`。架构说明见仓库根 [README.md](../README.md)。

## 环境准备

- **工程目录**：所有 Gradle 命令在 `1.20.1-forge/` 下执行，不要在仓库根目录跑 `gradlew`；
- **JDK 17**：模组编译目标（`java.toolchain.languageVersion = 17`）；
- **JDK 21**：ModDevGradle 工件生成需要；通过 `gradle.properties` 的 `org.gradle.java.installations.paths` 指定（示例：`D:/java21`）；
...
1. 从版本分支拉取最新（1.20.1 为 `1.20.1-forge`），再创建功能分支：`feature/<主题>`；
2. 实现并补充测试（改动只放进对应版本目录）；
3. 在 `1.20.1-forge/` 本地验证：`gradlew test build` 必须全绿；涉及游戏内行为的改动运行 `gradlew runGameTestServer`；
...
- 第三方接入统一走 `MCT.runtime()`，未安装 MC Thread 时返回 NOOP。
