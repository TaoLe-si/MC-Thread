...
import com.taolesi.threadtearer.runtime.InteractionRequest;
...
public final class InteractionOffloadPipeline {
    ...
    /**
     * Interaction handed over from an offloaded tick compute (addon API). The
     * write is ordered on the interaction FIFO with player interactions, then
     * applied exactly once on the owner thread inside
     * {@link InteractionRelocator#runApply}. The returned future completes
     * after the owner applied (or failed); never join it from the owner
     * thread. The owner never waits.
     */
    public CompletableFuture<OffloadOutcome> scheduleWorldApply(String request, Runnable apply) {
        submitted.incrementAndGet();
        CompletableFuture<OffloadOutcome> done = new CompletableFuture<>();
        interactionExecutor.execute(new InteractionRequest(request, () -> {
            lastEmitThread.set(Thread.currentThread().getName());
            emitted.incrementAndGet();
            ownerExecutor.execute(() -> {
                long start = System.nanoTime();
                try {
                    InteractionRelocator.runApply(request, apply);
                    applyNanos.addAndGet(System.nanoTime() - start);
                    applied.incrementAndGet();
                    lastOutcome.set(OffloadOutcome.APPLIED);
                    done.complete(OffloadOutcome.APPLIED);
                } catch (Throwable t) {
                    applyNanos.addAndGet(System.nanoTime() - start);
