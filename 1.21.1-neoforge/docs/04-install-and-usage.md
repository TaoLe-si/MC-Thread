...
- Minecraft **1.21.1** + NeoForge **21.1.x**（本工程锁定 `neo_version=21.1.250`）；
- Java 21（本模组编译目标 Java 21）。

## 2. 安装到客户端

1. 安装 NeoForge 1.21.1：从 neoforge.net 下载对应安装器，选择 **Install client**；
2. 找到游戏目录的 `mods` 文件夹：
...
4. 启动游戏，选择 **NeoForge** 配置；主菜单的"模组"列表应显示 **线程撕裂者**；
...
| Logger | 内容 |
| --- | --- |
| `[Thread Tearer]` | 模组生命周期（加载、服务器启动/停止、适配器挂载） |
